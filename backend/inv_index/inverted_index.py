...
def calculate_inverted_index(wiki_pages, output: str, normalize: bool = True):
    ...
    with open("backend/inv_index/corpus_wiki.json", "r") as file:
        data = json.load(file)
    ...
    with open(f"backend/inv_index/{output}", "w", encoding="utf-8") as output:
        json.dump(inverted_index, output, indent=1, sort_keys=True, ensure_ascii=False)
...
if __name__ == "__main__":
    wiki_pages = ["Harry Potter", "Albus Dumbledore", "Tom Riddle", "Minerva McGonagall", "Severus Snape", "Schloss Hogwarts"]
    output = "inv_index_wissenschaftskommunikation_normalized.json"
    calculate_inverted_index(wiki_pages, output, normalize=True)
