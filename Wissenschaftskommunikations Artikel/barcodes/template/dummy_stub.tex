...
\begin{document}
    \begin{landscape}
        ...
        \begin{figure}[!h]
            \centering
            \begin{tikzpicture}
                \node[anchor=south west, inner sep=0] (image) at (0,0) {\includesvg[scale=0.38]{\VAR{ file }}};
                \begin{scope}
                    [x={(image.north west)}, y={(image.south east)}]
...
% Legend
        \begin{tikzpicture}[overlay]
            \node[anchor=south west] at ([xshift=10cm, yshift=-7cm]image.north west) {
