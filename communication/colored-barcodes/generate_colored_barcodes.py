...
def generate_embedding_for_term(model: Word2Vec, term: str, idx: int | str, questions_set: bool = False, answers_set: bool = False):
    ...
    if questions_set:
        QUESTIONS_DIR = os.path.abspath(os.path.join(BASE_DIR, "questions"))
        _ensure_directory_exists(QUESTIONS_DIR)
        save_plot(path_to_save=f"{QUESTIONS_DIR}/question", idx=idx)

    if answers_set:
        ANSWERS_DIR = os.path.abspath(os.path.join(BASE_DIR, "answers"))
        _ensure_directory_exists(ANSWERS_DIR)
        save_plot(path_to_save=f"{ANSWERS_DIR}/answer", idx=idx)
...
if __name__ == "__main__":
    # Load the model
    model = Word2Vec.load(f"{BASE_DIR}/harry_potter_german_word2vec.model")
