...
export class DataService {
  ...
  public async getQueryResults(query: String): Promise<QueryResponseResult[]> {
    const response = await lastValueFrom(
      this.httpClient.get<QueryResponseResult[]>(
        `${environment.baseUrl}/search?q=${query}`
      )
    );
  ...
  public async getDocumentById(id: number) {
    return await lastValueFrom(
      this.httpClient.get<QueryResponseDocument>(
        `${environment.baseUrl}/document?id=${id}`
      )
    );
  }
  ...
  public async getWordEmbedding(paragraph: String, query: String) {
    return await lastValueFrom(
      this.httpClient.post<WordEmbedding[]>(
        `${environment.baseUrl}/word_embeddings`,
        { paragraph: paragraph, query: query }
      )
    );
  }
}
